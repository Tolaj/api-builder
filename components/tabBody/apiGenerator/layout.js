...
const MyCompProfile = params => {
  ...
  const payloadCell = (params)=>{
    ...
    const handleDelete = (index) => {
        const updatedPayload = params.data.payload.filter((_, i) => i !== index);
        params.node.setDataValue('payload', updatedPayload);
    };
   
    if(params.rowIndex != rowIndex ){
        if(params.data.payload.length == 0){
            return(
                <div className=" float-left pb-5 pt-2 ">
        ...
        {params.data.payload.map((item, index) => {
            return (
                <div class="flex-shrink-0 inline-flex max-w-fit items-center px-2 py-1 mr-2 text-xs text-gray-800 bg-[#F0BD66] rounded">
                    <span onClick={()=>{setRowIndex(params.rowIndex)}} className=" hover:cursor-pointer">{item.fieldName} - {item.dataType}</span>
                    <button onClick={()=>{handleDelete(index)}} type="button" class="inline-flex items-center p-1 ml-2 text-sm text-gray-400 bg-transparent rounded-sm hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-gray-300" data-dismiss-target="#badge-dismiss-dark" aria-label="Remove">
        ...
        {params.data.statusCode.map((item, index) => {
            return (
                <div class="flex-shrink-0 inline-flex max-w-fit items-center px-2 py-1 mr-2 text-xs text-gray-800 bg-slate-300 rounded">
                    <span className=" hover:cursor-pointer">{item.code} - {item.remark}</span>
